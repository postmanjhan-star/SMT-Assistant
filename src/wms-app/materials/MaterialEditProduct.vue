...
import { ApiError, MaterialsService, OpenAPI, ProductUpdate, UnitEnum } from '../../client'
...
const formValue = ref<ProductUpdate>( {
  id: 0,
  idno: '',
  name: '',
  description: '',
...
async function onSubmitMaterialForm ( event: Event ) {
  ...
  // Update Material
  try {
    const response = await MaterialsService.updateProduct( { id: formValue.value.id, requestBody: formValue.value } );
