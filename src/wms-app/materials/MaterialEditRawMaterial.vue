...
import { FormInst, FormRules, NButton, NForm, NFormItemGi, NGrid, NInput, NInputNumber, NSelect, NSpace, SelectOption, useMessage } from 'naive-ui'
import { onBeforeMount, ref } from 'vue'
import { useRoute, useRouter } from 'vue-router'
import { ApiError, MaterialsService, OpenAPI, RawMaterialUpdate, UnitEnum } from '../../client'
...
const formValue = ref<RawMaterialUpdate>( {
  id: 0,
  idno: '',
  name: '',
  description: '',
...
async function onSubmitMaterialForm ( event: Event ) {
  ...
  // Update Material
  try {
    const response = await MaterialsService.updateRawMaterial( { id: formValue.value.id, requestBody: formValue.value } );
    message.success( `更新成功` );
    router.push( '/wms/materials' );
  } catch ( error ) {
      ...
      <n-grid cols="1 s:3" responsive="screen" x-gap="20">
        ...
        <n-form-item-gi show-require-mark label="基本包裝量">
          <n-input-number v-model:value.lazy=" formValue.qty_per_pack " :show-button=" false " :min=" 1 "
            style="width: 100%;" :precision=" 0 " :default-value=" 1 " id="qty_per_pack"></n-input-number>
        </n-form-item-gi>
        ...
        <n-form-item-gi show-require-mark label="有效期間">
          <n-input-number v-model:value.lazy=" formValue.expiry_days " :show-button=" false " :min=" 1 "
            style="width: 100%;" :precision=" 0 " id="expiry_days">
