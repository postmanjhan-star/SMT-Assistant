...
async function onClickCreateIssuanceReturnButton ( event: Event ) {
  ...
  try {
    inventory = await MaterialInventoriesService.getMaterialInventory( { materialInventoryIdno: issuanceReturnCreationFormValue.value.materialInventoryIdno.trim() } )
    ...
    router.push( '/wms/issuance_returns' );
  } catch ( error ) {
...
<template>
  <main
    ...
    <n-breadcrumb
      ...
      <n-breadcrumb-item>
        <router-link to="/wms/issuance_returns" #=" { navigate, href } " custom>
