...
const formValue = ref<InProcessMaterialCreate>( {
  idno: '',
  material_type: MaterialTypeEnum.IN_PROCESS_MATERIAL,
  name: '',
  description: '',
...
async function onSubmitMaterialCreationForm ( evnet: Event ) {
  ...
  // Create Material
  try {
    const response = await MaterialsService.createInProcessMaterial( { requestBody: formValue.value } );
...
<template>
  <main
    ...
    <n-breadcrumb
      ...
      <n-breadcrumb-item>建立半成品</n-breadcrumb-item>
    </n-breadcrumb>

    <div style="padding: 1rem;">
      <n-h1 prefix="bar" style="font-size: 1.4rem;">建立半成品</n-h1>
