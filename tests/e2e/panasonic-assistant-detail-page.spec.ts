...
test('panasonic detail: 掃 S5566 進入接料模式，接料輸入框出現', async ({ page }) => {
    ...
    await expect(page.getByTestId('panasonic-splice-material-input').locator('input')).toBeVisible();
...
test('panasonic detail: 接料完整流程 舊料→新料→站位 更新 appendedMaterialInventoryIdno', async ({ page }) => {
    await setupPanasonicSplicePage(page);
    await panasonicFirstLoad(page);
    ...
    const spliceMaterialInput = page.getByTestId('panasonic-splice-material-input').locator('input');
    const spliceSlotInput = page.getByTestId('panasonic-splice-slot-input').locator('input');
...
test('panasonic detail: 接料進行中退出接料模式，correct 狀態還原為 ✅', async ({ page }) => {
    ...
    await expect(row.locator('[col-id="correct"]')).toContainText('✅');
    await expect(page.getByTestId('panasonic-splice-material-input').locator('input')).not.toBeVisible();
